...
const ACCOUNTS_INFO = "https://sandbox.apihub.citi.com/gcb/api/v1/customer";
...
const CLIENT_SECRET = "S4sK0dU7oN0fG8wR7tE7qK5nD5eU2dD5aX4lR8qX1jK5wH1vR3";
...
app.use(express.static(__dirname + '/FreelanceStatic'));
app.use("/scripts", express.static(__dirname + '/FreelanceStatic/Scripts'));
		...
		async.waterfall( //synchronous calls to get token first, then account
			...
			function(err, successfulAccount){
				...
				if(err){
					...
					// 	'</pre>'
					// 	);
					fs.writeFile("FreelanceStatic/UserInfo.json", successfulAccount, function(err) {
					    if(err) {
					        return console.log(err);
					    }
					});
					res.sendFile(path.join( __dirname, 'FreelanceStatic', 'freelance.html' ));
					// res.redirect('FreelanceStatic/freelance.html');
				}
			}
		);
	}
...
// function fetchInfo(token, callback) {
// 	access_token = "Bearer " + token;
// 	axios({
// 		method: 'get',
// 		url: ACCOUNT_INFO,
...
// 		}
// 	}).then(function(response){
// 		var successfulInformation = JSON.stringify(response.data, undefined, 2);
// 		callback(null, successfulInformation);
// 	}).catch(function(error){
// 		callback(error, null);
// 	});
// }
